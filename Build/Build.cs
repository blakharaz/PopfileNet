...
<!-- coverage-table-end -->";
    ...
    Target GenerateRelease => _ => _
        ...
        .Executes(() =>
        {
            StartShell("dotnet tool install -g aspire.cli --prerelease").AssertZeroExitCode();

            var appHostProject = RootDirectory / "PopfileNet.AppHost" / "PopfileNet.AppHost.csproj";
            StartShell($"dotnet add \"{appHostProject}\" package Aspire.Hosting.Docker").AssertZeroExitCode();

            var appHostPath = RootDirectory / "PopfileNet.AppHost" / "AppHost.cs";
            var originalAppHost = System.IO.File.ReadAllText(appHostPath);

            var tempAppHost = @"var builder = DistributedApplication.CreateBuilder(args);

builder.AddDockerComposeEnvironment(""popfilenet"");
...
builder.Build().Run();
";
            System.IO.File.WriteAllText(appHostPath, tempAppHost);
            ...
            {
                System.IO.File.WriteAllText(appHostPath, originalAppHost);
            }
        });
